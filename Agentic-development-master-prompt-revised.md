...
- **Storage (Vector RAG, Knowledge Graph + Relational + Caching):**
  - Storage is primarily local file-based and embedded, with the explicit exception of the cloud-based Knowledge Graph.
  ...
  - **Knowledge Graph (GraphRAG):** **Neo4j AuraDB Free** (Cloud-managed) for semantic entity relationships and multi-hop reasoning. Note: The architecture must account for the free tier limits of 50,000 nodes and 175,000 relationships. Alternatively, embedded **Kùzu** or local file-persisted NetworkX/RDFlib can be used if offline operation is required.
...
- **Backend-Only LLM Routing & LiteLLM:** 
  ...
  - Embedding model interactions should use Langchain integrations.
...
- Pre-flight checks must verify: Python virtual environment (`venv`), Node.js 20.x (if UI included), required environment variables (including Neo4j AuraDB credentials), local Vector DB / SQLite directory permissions, and LLM gateway reachability. Fail fast with clear error messages.
...
- Structured JSON logging (e.g., `structlog` in Python, `pino` in Node.js) that records for every agent action, graph query, and LLM call: latency, token usage, model used, cache hit/miss, with automatic secret/PII redaction.
...
- Strict separation of concerns:
  ...
  - RAG ingestion, Knowledge Graph extraction & DB query services
...
- **Universal Grounding Panel (Vector + Graph RAG):** 
  - A dedicated UI section showing current vector embedding stats and Knowledge Graph entity/relationship counts.
  - A visualization widget (e.g., React Force Graph) to view the localized Knowledge Graph.
  - Controls to upload/embed additional documents dynamically.
  - **Rule:** This embedded context (Semantic + Graph) MUST be injected as mandatory grounding context into *every* subsequent LLM call made by the system.
...
1. **`prd.md` (Product Requirements Document):** Generate an industry-standard PRD. This will serve as the master reference and source of truth for all subsequent coding phases. It must include:
   ...
   - **AI & System Architecture:** High-level AI capabilities, agent topology, and RAG/GraphRAG strategy.
   - **Functional Requirements:** Strict capabilities the system must possess, including the mandated UI elements (Header Monitor, Settings Modal, RAG/GraphRAG Grounding).
   - **Non-Functional Requirements:** Performance, security, constraints (e.g., zero-admin, TLS-bypass, local file-based storage, Neo4j connection handling).
...
5. Integration strategy for LLM routing & AgentOps (including TLS bypass handling).
6. Agent topology & workflow definition (Framework-specific).
7. ChromaDB, Knowledge Graph (Neo4j AuraDB schema definitions & extraction pipelines), & SQLite schema definitions.
8. Caching strategy (Local Disk Cache / SQLite).
9. Centralised logging & observability strategy.
10. UI component / design-token architecture (including the Header Monitor, Graph Viz, and Settings Drawer).
...
2. **Model Mapping & Structured Output Strategy**  
   Map the available models to specific agent roles. Explain how strict JSON output will be enforced (Pydantic / Zod validation).

3. **Hybrid RAG (Vector + Knowledge Graph) Strategy**  
   Explain how ChromaDB will handle local embeddings and how Neo4j AuraDB will be utilized for graph extraction and traversal (accounting for the 50k node/175k relationship limits). Explain how hybrid grounding will be enforced on all calls.

4. **Constraint & TLS Confirmation**  
   Confirm how the architecture satisfies user-space execution, unprivileged ports, embedded storage (with the Neo4j cloud exception), Langchain backend routing, A2A readiness, and TLS-bypass rules. Note security trade-offs.

5. **Operations & Startup Plan**  
   Outline the approach for pre-flight checks (`venv`, ports, DB files, Neo4j credentials), retries, structured logging, and graceful shutdown.
...
7. **UI Design Token Approach**  
   Confirm the token-based theme strategy and how the required hackathon UI elements (Monitor, Config Modal, Graph Viz) will interact with the backend API.
